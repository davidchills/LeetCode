/*
A digit string is good if the digits (0-indexed) at even indices are even and the digits at odd indices are prime (2, 3, 5, or 7).
For example, "2582" is good because the digits (2 and 8) at even positions are even and the digits (5 and 2) at odd positions are prime. 
However, "3245" is not good because 3 is at an even index but is not even.
Given an integer n, return the total number of good digit strings of length n. Since the answer may be large, return it modulo 10^9 + 7.
A digit string is a string consisting of digits 0 through 9 that may contain leading zeros.
    
1922. Count Good Numbers    
*/
class Solution {
    func countGoodNumbers(_ n: Int) -> Int {
        let MOD = Int(1e9 + 7)
        let evenPositions = (n + 1) / 2
        let oddPositions = n / 2

        let evenWays = modPow(5, evenPositions, MOD)
        let oddWays = modPow(4, oddPositions, MOD)
        
        return (evenWays * oddWays) % MOD
    }
    

    private func modPow(_ base: Int, _ exp: Int, _ mod: Int) -> Int {
        var result = 1
        var base = base % mod
        var exp = exp
        
        while exp > 0 {
            if exp % 2 == 1 {
                result = (result * base) % mod
            }
            base = (base * base) % mod
            exp /= 2
        }
        return result
    }    
}


let solution = Solution()
print(solution.countGoodNumbers(1)) // Expect 5
print(solution.countGoodNumbers(4))  // Expect 400
print(solution.countGoodNumbers(50))  // Expect 564908303