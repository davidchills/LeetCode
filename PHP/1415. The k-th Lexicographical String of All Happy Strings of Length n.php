...
class Solution {
    ...
    public function getHappyString(int $n, int $k): ?string {
        ...
        $path = [];
        $count = 0;
        return $this->backtrack($n, $path, $letters, $k, $count) ?? "";
    }
    
    private function backtrack(int $n, array &$path, array $letters, int $k, int &$count): ?string {
        if (count($path) === $n) {
            $count++;
            if ($count === $k) {
                return implode("", $path);
        ...
        foreach ($letters as $letter) {
            if (empty($path) || end($path) !== $letter) {
                $path[] = $letter;
                $result = $this->backtrack($n, $path, $letters, $k, $count);
                array_pop($path);
                if ($result !== null) {
                    return $result;
                }
